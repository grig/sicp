...
(define (get-record division employee-name)
  ((get 'get-record (tagged-by division)) (contents division) employee-name))

;; each division should provide a get-record implementation working with this division's file format.
;; division files should themselves be tagged with division name; this could be done by consing division name to division file itself.
...
;; employee records returned by get-record should be tagged with division name.

(define (get-salary employee)
  ((get 'get-salary (tagged-by employee)) (contents employee)))
...
;; - Ensure that employee files have compatible format,
;; - implement get-record for this file format
;; - implement other functions such as get-salary working on individual employee records.
